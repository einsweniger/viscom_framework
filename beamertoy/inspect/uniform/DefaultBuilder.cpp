...
#include "BooleanUniform.h"
#include "UnsignedUniform.h"

namespace models {
    std::unique_ptr<generic_uniform> DefaultBuilder::operator()(named_resource res) {
        using namespace glwrap;
        switch (static_cast<resource_type>(res.properties.at(gl::GL_TYPE))) {
            case glsl_float: {
                auto uni = FloatUniform(std::move(res));
                uni.value_upload_fn = upload_float1(uni);
                uni.draw_value_fn = drag_float1(uni);
                return std::make_unique<FloatUniform>(std::move(uni));
            }
            case glsl_vec2:{
                auto uni = FloatUniform(std::move(res));
                uni.value_upload_fn = upload_float2(uni);
                uni.draw_value_fn = drag_float2(uni);
                return std::make_unique<FloatUniform>(std::move(uni));
            }
            case glsl_vec3:{
                auto uni = FloatUniform(std::move(res));
                uni.value_upload_fn = upload_float3(uni);
                uni.draw_value_fn = drag_float3(uni);
                return std::make_unique<FloatUniform>(std::move(uni));
            }
            case glsl_vec4: {
                auto uni = FloatUniform(std::move(res));
                uni.value_upload_fn = upload_float4(uni);
                uni.draw_value_fn = drag_float4(uni);
                return std::make_unique<FloatUniform>(std::move(uni));
            }
            case glsl_double:
            case glsl_dvec2:
