    ...
    for( let i=0; i<4; i++ )
    {
        ...
        var bar = document.getElementById( "inputSelectorControls" + i);
          ...
          can.onclick = function (ev)
          {
            ...
            z.className = "uiButton";
            z.style.visibility = 'hidden';
            z.onclick = function(ev) { var ele = piGetSourceElement(ev); var r = me.MuteInput( i ); if( r===true ) ele.src="/img/themes/" + gThemeName + "/speakerOff.png"; else ele.src="/img/themes/" + gThemeName + "/speakerOn.png";}
            bar.appendChild( z );
...
function ShaderToy( parentElement, editorParent, passParent )
{
    ...
    {
        ekeys = { "Ctrl-S":    function(instance) { doSaveShader(); me.mNeedsSave=false; } ,
                  "Cmd-S":     function(instance) { doSaveShader(); me.mNeedsSave=false; } ,
                  "Alt-Enter": function(instance) { me.SetShaderFromEditor(false); },
                  "Cmd-Enter": function(instance) { me.SetShaderFromEditor(false); },
    ...
    this.mCodeEditor.on( "change",         function(instance,ev)        { me.mEditorState.mCodeChange=true; me.mNeedsSave = true; me.mPass[me.mActiveDoc].mDirty=true; me.mPass[me.mActiveDoc].mCharCountDirty = true; } );
        ...
        for( var i=0; i<lines.length; i++ )
        {
            ...
            if( parts.length===5 || parts.length===6 )
            {
                ...
                if( isNaN(lineAsInt) ) // for non-webgl errors
                {
                  ...
                  var mark = this.mCodeEditor.addLineWidget( 0, msg, {coverGutter: false, noHScroll: true} );
                ...
                else
                {
                  ...
                  var mark = this.mCodeEditor.addLineWidget( lineNumber, msg, {coverGutter: false, noHScroll: true} );
                  this.mErrors.push( mark );
                }
            }
...
ShaderToy.prototype.showChars = function()
{
    ...
    doAlert( piGetCoords(ve), {mX:480,mY:400}, "Minimal Shader Code, (" + str.length + " chars)", "<pre>"+htmlEntities(str)+"</pre>", false, null );
}
...
*/
        ...
        for( var i=0; i<num; i++ )
        {
            this.mPass[i] = { mDocs:   CodeMirror.Doc( res[i].mShader, "text/x-glsl"),
...
function bbc2html( content, allowMultimedia )
{
    ...
    content = content.replace( /(\[url=)(.*?)(\])(.*?)(\[\/url\])/g, '<a href="$2" class="regular" target="_blank">$4</a>');
    content = content.replace( /(\[url\])(.*?)(\[\/url\])/g,         '<a href="$2" class="regular" target="_blank">$2</a>');
    ...
    content = content.replace( /(:octopus:)/g,            '<img src="/img/emoticonOctopus.png">' );
    content = content.replace( /(:octopusballoon:)/g,     '<img src="/img/emoticonOctopusBalloon.png">' );
    ...
    content = content.replace( /(\[b\])([.\s\S]*?)(\[\/b\])/g,       '<strong>$2</strong>' );
    content = content.replace( /(\[B\])([.\s\S]*?)(\[\/B\])/g,       '<strong>$2</strong>' );
    ...
    content = content.replace( /(\[code\])([.\s\S]*?)(\[\/code\])/g, '<pre>$2</pre>'       );
    content = content.replace( /(\[Code\])([.\s\S]*?)(\[\/Code\])/g, '<pre>$2</pre>'       );
    content = content.replace( /(\[CODE\])([.\s\S]*?)(\[\/CODE\])/g, '<pre>$2</pre>'       );

    if( allowMultimedia )
    {
        content = content.replace( /(\[img\])(.*?)(\[\/img\])/g, '<a href="$2"><img src="$2" style="max-width:100%;"/></a>' );
        content = content.replace( /(\[Img\])(.*?)(\[\/Img\])/g, '<a href="$2"><img src="$2" style="max-width:100%;"/></a>' );
        content = content.replace( /(\[IMG\])(.*?)(\[\/IMG\])/g, '<a href="$2"><img src="$2" style="max-width:100%;"/></a>' );
        content = content.replace( /(\[video\])(?:http|https|)(?::\/\/|)(?:www.|)(?:youtu\.be\/|youtube\.com(?:\/embed\/|\/v\/|\/watch\?v=|\/ytscreeningroom\?v=|\/feeds\/api\/videos\/|\/user\S*[^\w\-\s]|\S*[^\w\-\s]))([\w\-]{11})(.*?)(\[\/video\])/g, '<iframe width="100%" height="360" src="https://www.youtube.com/embed/$2?hd=1" frameborder="0" allowfullscreen></iframe>' );
    }
...
function dataLoadShader( jsnShader )
{
    ...
    if( shaderAuthorName ) shaderAuthorName.innerHTML = "<a class='user' href='/user/" + htmlEntities(gRes.mUserName) +"'>" + htmlEntities(gRes.mUserName) + "</a>";
...
function updatepage( jsn )
{
    ...
    for( var i=0; i<numComments; i++ )
    {
        ...
        txt +=   "<td class=\"commentContent\"><a class='user' href='/user/"+ htmlEntities(jsn.username[i]) +"'>" + htmlEntities(jsn.username[i]) + "</a>, " + timeVar + "<br>" + bbc2html(htmlEntities(jsn.text[i]),true) + "</td></tr></table>";
        txt += "</div>";
    }
...
function addComment(usr, form)
{
    ...
    {
        var ve = document.getElementById("centerScreen" );
        doAlert( piGetCoords(ve), {mX:320,mY:100}, "Error", "We are sorry, we couldn't submit your comment", false, null );
...
function checkFormComment(str)
{
    ...
    {
        var ve = document.getElementById("centerScreen" );
        doAlert( piGetCoords(ve), {mX:320,mY:100}, "Error", "You need to write at least 1 character", false, null );
        return false;
    }
    return true;
}
...
function shaderSaved( res, savedID, isUpdate, dataJSON )
{
    ...
    {
        doAlert( piGetCoords(ve), {mX:400,mY:160}, "Error", "Shader name \"" +  dataJSON.info.name + "\" is already used by another shader. Please change the name of your shader.", false, null );
    }
    else if( res===-3 )
    {
        doAlert( piGetCoords(ve), {mX:400,mY:180}, "Error", "The shader could not be " + ((isUpdate==true)?"updated":"added") + ", you might not be logged in anymore, please Sign In again.", false, null);
    }
    else if (res === -13)
    {
        doAlert(piGetCoords(ve), { mX: 400, mY: 180 }, "Error", "The shader coubld not be saved, it is using private assets.", false, null);
    }
    else
    {
        doAlert( piGetCoords(ve), {mX:400,mY:180}, "Error", "The shader could not be " + ((isUpdate==true)?"updated":"added") + ", please try again. Error code : " + res, false, null);
    }
}
...
function openSubmitShaderForm( isUpdate )
{
    ...
    if( !s1.validity.valid ) { doAlert( piGetCoords(ve), {mX:320,mY:100}, "Error", "You must give a name to your shader", false, null ); return; }
    if( !s2.validity.valid ) { doAlert( piGetCoords(ve), {mX:320,mY:100}, "Error", "You must assign at least one tag to your shader", false, null ); return; }
    if( !s3.validity.valid ) { doAlert( piGetCoords(ve), {mX:320,mY:100}, "Error", "You must give a description to your shader", false, null ); return; }
  ...
  //if( !checkFormComment(s1.value) ) return false;
    ...
    if( publishedStatus==1 || publishedStatus==3 )
    {
        ...
        {
            var ve = document.getElementById("centerScreen" );
            doAlert( piGetCoords(ve), {mX:400,mY:160}, "Error", "The shader can not be saved because it does not compile", false, null );
            return;
        }
    }
