 ...
 */
MyFreeCamera::MyFreeCamera(const glm::vec3& theCamPos,
...
bool MyFreeCamera::HandleMouse(int button, int action, float mouseWheelDelta,
 ...
 */
void MyFreeCamera::UpdateCamera(double elapsedTime,
                                const ApplicationNodeBase* sender) {
  ...
  if (moveLength > glm::epsilon<float>())
    camMove =
        (camMove / moveLength) * static_cast<float>(moveSpeed * elapsedTime);
  ...
  pitch = glm::clamp(pitch, -glm::half_pi<float>() * 0.99f,
                     glm::half_pi<float>() * 0.99f);
  //        currentPY_.x = glm::clamp(currentPY_.x, -glm::half_pi<float>() *
  //        0.99f, glm::half_pi<float>() * 0.99f);
  auto newOrientation = glm::quat(glm::vec3(pitch, 0.0f, 0.0f)) *
                        glm::quat(glm::vec3(0.0f, yaw, 0.0f)) *
 ...
 */
void MyFreeCamera::SetMoveSpeed(double speed) { moveSpeed_ = speed; }

/**
 * Get the current speed of camera movement
 * @return speed value
 */
double MyFreeCamera::GetMoveSpeed() { return moveSpeed_; }

void MyFreeCamera::resetMouse() { firstRun_ = true; }
