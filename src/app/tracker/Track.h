...
static ImVec2 element_size() { return ImVec2(100, ImGui::GetTextLineHeight()); }

enum class interpolation_type { Step, Linear, Smooth, Ramp };
...
constexpr interpolation_type rotateInterpolation(interpolation_type type) {
  switch (type) {
    ...
    case interpolation_type::Smooth:
      return interpolation_type::Ramp;
...
static ImVec4 get_text_color(tracker::interpolation_type type) {
  switch (type) {
    ...
    case tracker::interpolation_type::Smooth:
      return ImVec4(0.000f, 1.000f, 0.000f, 1.000f);
    case tracker::interpolation_type::Ramp:
      return ImVec4(0.000f, 0.618f, 1.000f, 1.000f);
  }
  return ImVec4(1, 1, 1, 1);
}
...
template <class KeyType>
void AbstractTrack<KeyType>::draw_empty_selectable(Row active_row,
                                                   interpolation_type type) {
...
template <class KeyType>
void AbstractTrack<KeyType>::draw_value_selectable(Row active_row,
                                                   KeyType &key) {
  bool focus = false;
  ImGui::PushID(static_cast<int>(active_row));
  if (ImGui::Selectable(value_str(key.value).c_str(), false, 0,
                        element_size())) {
    focus = true;
...
void AbstractTrack<KeyType>::drawTrack(ImGuiListClipper &clip) {
  while (clip.Step()) {
    ...
    Row clip_end = static_cast<Row>(clip.DisplayEnd);
    Row active_row = static_cast<Row>(clip.DisplayStart);
    Row upper = clip_end;
    if (!keys.empty()) {
