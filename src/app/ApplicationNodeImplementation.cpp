...
void ApplicationNodeImplementation::InitOpenGL() {
  ...
  freeCam_ = std::make_unique<MyFreeCamera>(GetCamera()->GetPosition(),
                                            *GetCamera(), 15);
  scriptCam_ = std::make_unique<ScriptedCamera>(GetCamera()->GetPosition(),
                                                *GetCamera(), this);
  ...
  auto texDescr = enh::TextureDescriptor{sizeof(float), gl::GL_R32F, gl::GL_RED,
                                         gl::GL_FLOAT};
  fftTex = std::make_unique<viscom::enh::GLTexture>(minuseins::audio::FFT_SIZE,
                                                    texDescr);
  fftTexSmoothed = std::make_unique<viscom::enh::GLTexture>(
      minuseins::audio::FFT_SIZE, texDescr);
  fftTexIntegrated = std::make_unique<viscom::enh::GLTexture>(
      minuseins::audio::FFT_SIZE, texDescr);

  bass->openFile(
      Resource::FindResourceLocation("media/song.ogg", GetApplication()));
  auto [bytes, time] = bass->get_length();
  std::cout << "bass length in bytes: " << bytes << " and in seconds: " << time
            << std::endl;
  std::cout << "that's: " << time * minuseins::audio::row_rate << " rows!"
            << std::endl;
  bass->play();
}

void ApplicationNodeImplementation::UpdateFrame(double currentTime,
                                                double elapsedTime) {
  //        if(stopTime_) {
  //            stopTime_ = !stopTime_;
  //            bass->play();
  ...
  if (bass->get_fft(&fftData[0])) {
    for (int i = 0; i < minuseins::audio::FFT_SIZE; i++) {
      fftDataSmoothed[i] = fftDataSmoothed[i] * fftSmootingFactor +
                           (1 - fftSmootingFactor) * fftData[i];

      fftDataSlightlySmoothed[i] =
          fftDataSlightlySmoothed[i] * fftSlightSmootingFactor +
          (1 - fftSlightSmootingFactor) * fftData[i];
...
bool ApplicationNodeImplementation::KeyboardCallback(int key, int scancode,
                                                     int action, int mods) {
  if (ApplicationNodeBase::KeyboardCallback(key, scancode, action, mods)) {
    return true;
  }
...
fs::path ApplicationNodeImplementation::findConfig(
    const std::string& config_name) {
  try {
    auto filename =
        Resource::FindResourceLocation(config_name, GetApplication());
    return {filename};
  } catch (resource_loading_error&) {
    auto resdir =
        fs::path{GetApplication()->GetConfig().resourceSearchPaths_.at(0)};
...
std::string ApplicationNodeImplementation::get_named_track_value(
    const std::string& name) {
  return namedTracks[name].get_value(bass->get_row());
}

std::vector<float> ApplicationNodeImplementation::get_track_vec(
    const std::string& name) {
