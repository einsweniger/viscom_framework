...
FileSelect::FileSelect(const std::string& name,
...
void FileSelect::draw(bool* p_open) {
  ...
  auto childsize =
      ImVec2(ImGui::GetWindowContentRegionWidth() / basepaths_.size(), 0);
  filter.Draw();
  for (auto it = basepaths_.begin(); it != basepaths_.end(); ++it) {
    auto& base = (*it);
    ImGui::PushID(base.c_str());
    ImGui::BeginChild(base.string().c_str(), childsize, true,
      ...
      if (filter.PassFilter(path.string().c_str())) {
        if (ImGui::SmallButton(path.filename().string().c_str())) {
          if (fs::is_directory(path)) {
            currentPath[base.string()] =
                currentPath[base.string()] / path.filename();
            pathContents[base.string()] =
                scan(base / currentPath[base.string()]);
            break;
