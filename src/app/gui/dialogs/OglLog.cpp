...
std::string build_string(const glbinding::FunctionCall &call) {
  ...
  for (unsigned i = 0; i < call.parameters.size(); ++i) {
    ...
    if (i < call.parameters.size() - 1) callOut << ", ";
  }
  callOut << ")";

  if (call.returnValue) callOut << " -> " << call.returnValue.get();
  callOut << "\n";
  return callOut.str();
}
...
void OglLog::callback(const glbinding::FunctionCall &call) {
  ...
  if (gl::GL_NO_ERROR != error) {
    ...
    auto errstr =
        std::string("Error: ") + glbinding::aux::Meta::getString(error) + "\n";
...
void OglLog::Draw(const char *title, bool *p_open) {
  if (*p_open && visible) ShaderLog::Draw(title, p_open);
}
}  // namespace minuseins::gui
