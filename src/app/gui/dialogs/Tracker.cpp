...
Tracker::Tracker(viscom::ApplicationNodeImplementation* appImpl)
    : appImpl(appImpl) {
  auto tmp = appImpl->bass->output_;
  decoder = std::make_unique<audio::BassDecoder>(appImpl->bass->openFile(
      viscom::Resource::FindResourceLocation("media/song.ogg",
                                             appImpl->GetApplication()),
      BASS_STREAM_DECODE | BASS_SAMPLE_MONO | BASS_POS_SCAN));
...
void drawInvisibleTracks(Tracks& tracks) {
  for (auto& track : tracks) {
    ...
    ImGui::SameLine();
    if (ImGui::Selectable(track.first.c_str(), false, 0,
                          tracker::element_size())) {
...
void Tracker::draw_table() {
  ...
  for (auto& tex : decoder->textures) {
    ...
    auto tex_height =
        seconds_per_image * rows_per_second * ImGui::GetTextLineHeight();
...
template <class Tracks>
void drawTrackContent(Tracks& tracks, bool track_active_row,
                      tracker::Row currentRow) {
...
void Tracker::draw_table_content() {
  ...
  //            ImGuiListClipper clip(row_count, tracker::element_size().y);
  //            if(track_active_row) {
  //                auto offset = appImpl->currentRow *
  //                tracker::element_size().y;
