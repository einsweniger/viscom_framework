...
MasterNodeGui::MasterNodeGui(viscom::ApplicationNodeImplementation *appImpl,
    ...
    : appImpl{appImpl}, appNode{appNode} {
  try {
    auto config_file =
        viscom::Resource::FindResourceLocation(config_name, appNode);
...
void MasterNodeGui::cleanup() {
  {  // set error callbacks
    using namespace glbinding;
    glbinding::removeCallbackMask(CallbackMask::After |
...
void MasterNodeGui::Draw2D(viscom::FrameBuffer &fbo) {
  fbo.DrawToFBO([this]() {
    ...
    //                animManager.ShowAnimationMenu("AnimationManager",
    //                activeWindows["AnimationManager"]);
    //            }
...
void MasterNodeGui::drawShaderToySelectImport(bool *p_open) {
  if (!*p_open) return;
  static FileSelect select{
      "Import ShaderToy", appNode->GetConfig().resourceSearchPaths_,
      [this](fs::path path) { return ShaderToyCallback(path); }};
  select.draw(p_open);
}
...
void MasterNodeGui::drawShaderImport(bool *p_open) {
  ...
  static FileSelect shader_select(
      "Open Shader", appNode->GetConfig().resourceSearchPaths_,
      [&](fs::path path) {
        try {
...
void MasterNodeGui::drawTextureImportWindow(bool *p_open) {
  if (!*p_open) return;
  static FileSelect texture_select{
      "Open Texture", appNode->GetConfig().resourceSearchPaths_,
      [&](fs::path path) { return textureCallback(path); }};
...
bool MasterNodeGui::ShaderToyCallback(fs::path path) {
  // TODO clean loading!
  try {
    if (path.has_extension() && ".json" == path.extension().generic_string()) {
      auto realpath =
          viscom::Resource::FindResourceLocation(path.string(), appNode);
      ...
      for (auto &buf : loader->buffers) {
        ...
        auto outfile = fs::path{"shadertoy/" + loader->toy_->info.id + "/" +
                                buf.name + ".frag"};
...
void MasterNodeGui::drawTimeSlider(bool *p_open) {
  ...
  if (ImGui::Begin("##TimeSlider", p_open, flags)) {
    ...
    ImGui::DragInt("##Time", reinterpret_cast<int *>(&appImpl->currentRow), 1,
                   0, 7000);
...
void MasterNodeGui::drawAnimation(bool *p_open) {
  if (!*p_open) return;
  static gui::Animation anim(appNode->GetConfig().resourceSearchPaths_.at(0),
                             appNode->GetCamera());
  anim.draw(p_open);
}
...
void MasterNodeGui::drawGlobalVars(bool *p_open) {
  ...
  ImGui::DragFloat("fftSmoothing", &appImpl->fftSmootingFactor, 0.01f, 0.0f,
                   0.98f);
  ...
  ImGui::PlotHistogram("", appImpl->fftDataIntegrated.data(), 750, 0, nullptr,
                       0.0f, appImpl->fftMaxIntegralValue, ImVec2(width, 300));
  ImGui::PlotHistogram("", appImpl->fftData.data(), 750, 0, nullptr, 0.0f,
                       histogram_scale, ImVec2(width, 300));
  ImGui::PlotHistogram("", appImpl->fftDataSmoothed.data(), 750, 0, nullptr,
                       0.0f, histogram_scale, ImVec2(width, 300));
  ImGui::Separator();
