...
bool MasterNode::KeyboardCallback(int key, int scancode, int action, int mods) {
  if (ApplicationNodeImplementation::KeyboardCallback(key, scancode, action,
                                                      mods)) {
    return true;
  }
  ...
  switch (key) {
    ...
    case GLFW_KEY_O:
      ...
      return true;
    case GLFW_KEY_M:
      gui_->toggle("MainMenu");
      return true;
    case GLFW_KEY_B:
      gui_->toggle("Buffers");
      return true;
    case GLFW_KEY_G:
      gui_->toggle("GPUProgram");
      return true;
    case GLFW_KEY_F1:
      storeCamPosition();
      return true;
...
void MasterNode::InitOpenGL() {
  ...
  if (restore(findConfig(minuseins::gui::MasterNodeGui::config_name),
              gui_.get())) {
    gui_->init();
  }
}
