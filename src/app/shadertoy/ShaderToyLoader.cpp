...
ShaderToyLoader::ShaderToyLoader(fs::path path) {
  ...
  for (auto& pass : toy_->renderpass) {
    if ("image" == pass.type) {
      // assert(nullptr == image); //Shadertoys are assumed to only have one
      // image
      image = pass;
  ...
  for (auto& pass : toy_->renderpass) {
    ...
    if ("common" == pass.type) {
      ...
      outstream << VERSION << "#include <../" + pass.type + "/header.glsl>\n"
                << (hasCommon && "common" != pass.type
                        ? "#include <Common.frag>\n"
                        : "")
                << pass.code
  ...
  //                    active_fsq_ =
  //                    std::make_unique<minuseins::IntrospectableFsq>(out,this);
  //                } else {
