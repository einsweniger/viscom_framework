...
iChannel::iChannel(named_resource res, const shadertoy::Input &input,
...
struct iChannelResolution : FloatUniform {
  iChannelResolution(named_resource res,
                     viscom::enh::ApplicationNodeBase *appBase,
                     const shadertoy::Input &input)
      : FloatUniform(std::move(res)), appBase(appBase), input(input) {
...
struct fftSampler : empty_uniform {
  fftSampler(named_resource res, viscom::ApplicationNodeImplementation *appImpl)
      : empty_uniform(std::move(res)), appImpl(appImpl) {}

  size_t uploadSize() override { return sampler; }
...
struct tex_postproc : empty_uniform {
  tex_postproc(named_resource res,
               viscom::ApplicationNodeImplementation *appImpl)
      : empty_uniform(std::move(res)), appImpl(appImpl) {}
...
ShaderToySamplerBuilder::ShaderToySamplerBuilder(
    viscom::enh::ApplicationNodeBase *appBase,
...
std::unique_ptr<generic_uniform> ShaderToySamplerBuilder::operator()(
    named_resource res) {
  if ("tex_text" == res.name &&
      res.properties.at(gl::GL_TYPE) == gl::GL_SAMPLER_2D) {
    auto inp = shadertoy::Input{};
    inp.src = "/media/a/text.png";
    inp.ctype = "texture";
    inp.sampler.wrap = "repeat";
    inp.channel = samplerCounter++;
    return std::make_unique<detail::iChannel>(std::move(res), std::move(inp),
                                              appBase);
  }
  if ("tex_noise" == res.name &&
      res.properties.at(gl::GL_TYPE) == gl::GL_SAMPLER_2D) {
    auto inp = shadertoy::Input{};
  ...
  if (res.name.length() == 9) {  // ichannel0
    if (res.properties.at(gl::GL_TYPE) == gl::GL_SAMPLER_2D) {
      ...
      std::cout << "sampler with substring:" << substr << "followed by "
                << res.name.substr(8, 1) << std::endl;
      int channel = atoi(res.name.substr(8, 1).c_str());
      std::cout << "on channel:" << std::to_string(channel) << std::endl;
      if (substr == "iChannel") {
        auto it = std::find_if(
            std::begin(pass.inputs), std::end(pass.inputs),
            [&](shadertoy::Input &input) { return input.channel == channel; });
        if (std::end(pass.inputs) != it) {
          return std::make_unique<detail::iChannel>(std::move(res), (*it),
                                                    appBase);
        }
      }
    }
