...
SceneSubroutineHandler::SceneSubroutineHandler(
    gl::GLenum stage, viscom::ApplicationNodeImplementation *appImpl)
    : SubroutineUniformHandler(stage), appImpl(appImpl) {}

std::unique_ptr<named_resource> SceneSubroutineHandler::initialize(
...
void SceneSubroutineHandler::postInit(
    ProgramInspector &inspect,
    resource_handler::named_resource_container &res) {
...
SubroutineChanger::SubroutineChanger(
    gl::GLenum stage, const named_resource &res,
...
void SubroutineChanger::use_subroutine(const std::string &subname) {
  ...
  auto it =
      std::find_if(subroutines.begin(), subroutines.end(),
                   [&](const std::pair<gl::GLuint, std::string> &content) {
