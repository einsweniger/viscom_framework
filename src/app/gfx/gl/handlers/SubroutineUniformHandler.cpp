...
std::unique_ptr<named_resource> SubroutineUniformHandler::initialize(
  ...
  try {
    auto residx =
        inspect.GetResourceIndex(subroutineUniformEnum(stage), sub->name);
    auto& old_res =
        inspect.GetContainer(subroutineUniformEnum(stage)).at(residx);
    auto& old_uniform = dynamic_cast<SubroutineUniform&>(*old_res);
    sub->previous_active =
        old_uniform.subroutines.at(old_uniform.active_subroutine);
...
SubroutineUniform::SubroutineUniform(gl::GLenum stage, named_resource res)
    : named_resource(std::move(res)),
      ...
      num_compatible_subroutines{
          make_positive(properties.at(gl::GL_NUM_COMPATIBLE_SUBROUTINES))} {}
