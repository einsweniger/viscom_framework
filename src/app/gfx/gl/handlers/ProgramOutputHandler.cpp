...
std::unique_ptr<named_resource> ProgramOutputHandler::initialize(
    ProgramInspector &inspect, named_resource res) {
  return std::make_unique<ProgramOutput>(res);
}

void ProgramOutputHandler::postInit(ProgramInspector &inspect,
                                    named_resource_container &outputs) {
  try {
...
void ProgramOutput::draw2D() {
  ...
  std::string headerName =
      name + " tex idx: " + std::to_string(textureLocation);
