...
gl::GLint generic_uniform::block_index() {
  return properties.at(gl::GL_BLOCK_INDEX);
}

gl::GLint generic_uniform::location() { return properties.at(gl::GL_LOCATION); }

resource_type generic_uniform::type() {
  return interfaces::types::toType(properties.at(gl::GL_TYPE));
}

gl::GLuint generic_uniform::array_size() {
  return util::ensure_positive(properties.at(gl::GL_ARRAY_SIZE));
}
...
void FloatUniform::drawValue() {
  ...
  int color_flags = ImGuiColorEditFlags_HDR |
  ...
  if (t::glsl_float == type()) {
    ImGui::DragFloat(header.c_str(), &value[0], v_speed, v_min, v_max,
                     display_format, power);
  } else if (t::glsl_vec2 == type())
    ImGui::DragFloat2(header.c_str(), &value[0], v_speed, v_min, v_max,
                      display_format, power);
  else if (t::glsl_vec3 == type()) {
    // ImGui::DragFloat3(header.c_str(), &value[0], v_speed, v_min, v_max,
    // display_format, power);
    ImGui::ColorEdit3(header.c_str(), &value[0], color_flags);
  } else if (t::glsl_vec4 == type()) {
    // ImGui::DragFloat4(header.c_str(), &value[0], v_speed, v_min, v_max,
    // display_format, power);
...
void SamplerUniform::draw2D() {
  ...
  //        ImGui::InputInt2(name.c_str(), static_cast<int*>(&boundTexture),
  //        &textureUnit);
...
bool FloatUniform::upload_value() {
  ...
  if (resource_type::glsl_float == type())
    gl::glUniform1fv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_vec2 == type())
    gl::glUniform2fv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_vec3 == type())
    gl::glUniform3fv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_vec4 == type())
    gl::glUniform4fv(location(), array_size(), &value[0]);
  else {
    return false;
  }
  return true;
...
bool BooleanUniform::upload_value() {
  ...
  if (resource_type::glsl_bool == type())
    gl::glUniform1iv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_bvec2 == type())
    gl::glUniform2iv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_bvec3 == type())
    gl::glUniform3iv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_bvec4 == type())
    gl::glUniform4iv(location(), array_size(), &value[0]);
  else {
    return false;
  }
...
bool DoubleUniform::upload_value() {
  if (resource_type::glsl_double == type())
    gl::glUniform1dv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_dvec2 == type())
    gl::glUniform2dv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_dvec3 == type())
    gl::glUniform3dv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_dvec4 == type())
    gl::glUniform4dv(location(), array_size(), &value[0]);
  else {
    return false;
  }
  return true;
...
bool UnsignedUniform::upload_value() {
  if (generic_uniform::upload_value()) return true;
  if (resource_type::glsl_uint == type())
    gl::glUniform1uiv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_uvec2 == type())
    gl::glUniform2uiv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_uvec3 == type())
    gl::glUniform3uiv(location(), array_size(), &value[0]);
  else if (resource_type::glsl_uvec4 == type())
    gl::glUniform4uiv(location(), array_size(), &value[0]);
  return true;
}
