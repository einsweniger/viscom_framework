...
class ProgramInspector {
 public:
  ...
  explicit ProgramInspector(gl::GLuint programId,
                            const std::string& name = "_no_name_");

  void set_recompile_function(std::function<gl::GLuint()> fn);
  void draw_gui(bool* p_open,
                std::vector<gl::GLenum> draw_interfaces = all_interfaces);
  ...
  std::unordered_map<gl::GLenum, std::unordered_map<std::string, gl::GLuint>>
      name_to_resid{};
  ...
  gl::GLuint GetResourceIndex(gl::GLenum interface,
...
struct resource_handler {
  ...
  virtual std::unique_ptr<named_resource> initialize(ProgramInspector& inspect,
                                                     named_resource res) = 0;

  // called after iteration through all interfaces is finished, since some
  // things need further handling: UniformBlock information depends on Uniforms
  // being present. SubroutineUniforms also need Subroutine interface
  // information, list might go on.
  virtual void postInit(ProgramInspector& inspect,
                        named_resource_container& resources) = 0;

  // do things here you'd normally do between glUseProgram and glDraw*
  virtual void prepareDraw(ProgramInspector& inspect,
                           named_resource_container& resources) = 0;
};
}  // namespace minuseins