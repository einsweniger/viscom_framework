...
ResourceProperties Interface::GetProgramResourceiv(gl::GLuint index, const std::vector<gl::GLenum> &props) {
...
std::string Interface::GetProgramResourceName(const gl::GLuint index, const gl::GLint bufSize) {
...
std::string Interface::GetProgramResourceName(const gl::GLuint index) {
    auto length = GetProgramInterfaceiv(gl::GL_MAX_NAME_LENGTH);
    return GetProgramResourceName(index, length);
}

gl::GLuint Interface::GetProgramInterfaceiv(const gl::GLenum property) {
    gl::GLint result = 0;
...
gl::GLuint Interface::GetActiveResourceCount() {
    return GetProgramInterfaceiv(gl::GL_ACTIVE_RESOURCES);
}

std::vector<name_location_type_t> Interface::GetNameLocationType() {
