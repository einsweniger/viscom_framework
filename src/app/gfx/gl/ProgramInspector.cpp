...
namespace minuseins {
ProgramInspector::ProgramInspector(gl::GLuint programId,
                                   const std::string& name)
    : programId_(programId), name{name} {}
...
void ProgramInspector::draw_gui(bool* p_open,
                                std::vector<gl::GLenum> draw_interfaces) {
  if (!*p_open) {
    return;
  }
  ...
  if (ImGui::IsItemHovered()) {
    ...
    for (auto interface : all_interfaces) {
      gl::GLint count;
      gl::glGetProgramInterfaceiv(programId_, interface,
                                  gl::GL_ACTIVE_RESOURCES, &count);
      if (0 >= count) continue;
      ImGui::BulletText(
          "%s: %d", glbinding::aux::Meta::getString(interface).c_str(), count);
    }
    ImGui::EndTooltip();
  }
  ImGui::SameLine();
...
void ProgramInspector::setHandlerFunction(gl::GLenum interface,
                                          handler_fn hdl_fn) {
  std::cout << "add handler" << std::endl;
  handler_fns[interface] = hdl_fn;
}

ProgramInspector::handler_fn ProgramInspector::GetHandlerFunction(
    gl::GLenum interface) {
  try {
    return handler_fns.at(interface);
  } catch (std::out_of_range&) {
    return [](named_resource res) -> named_resource_ptr {
      return std::make_unique<named_resource>(std::move(res));
    };
  }
}

ProgramInspector::named_resource_container& ProgramInspector::GetContainer(
    gl::GLenum interface) {
  return containers.at(interface);
}

void ProgramInspector::setHandler(gl::GLenum interface,
                                  std::unique_ptr<resource_handler> hdl) {
...
interfaces::BasicInterface ProgramInspector::GetInterface(
    gl::GLenum interface) {
...
gl::GLuint ProgramInspector::GetResourceIndex(gl::GLenum interface,
                                              const std::string& name) {
  return name_to_resid.at(interface).at(name);
}

named_resource* ProgramInspector::GetByName(gl::GLenum interface,
                                            const std::string& name) {
