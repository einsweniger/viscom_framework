...
void IntrospectableFsq::init(viscom::enh::ApplicationNodeBase* appNode) {
  ...
  gpuProgram_ = appNode->GetGPUProgramManager().GetResource(
      "fullScreenQuad_" + fragmentShader,
      std::vector<std::string>{"fullScreenQuad.vert", fragmentShader});
  // gpuProgram_ = fsq_->GetGPUProgram();
  gpi_ = std::make_unique<ProgramInspector>(gpuProgram_->getProgramId(),
                                            fragmentShader);
  ...
  // gpi_->addHandler(gl::GL_UNIFORM,
  // std::make_unique<UniformHandler>(handlers::ExternalUniformBuilder(appBase)));
  gpi_->setHandler(gl::GL_UNIFORM,
                   std::make_unique<UniformHandler>(
                       handlers::ExternalUniformBuilder(appBase, params_)));
  gpi_->setHandler(gl::GL_PROGRAM_OUTPUT,
                   std::make_unique<ProgramOutputHandler>());
  gpi_->setHandler(gl::GL_UNIFORM_BLOCK,
                   std::make_unique<UniformBlockHandler>(appBase));
  gpi_->setHandler(gl::GL_FRAGMENT_SUBROUTINE_UNIFORM,
                   std::make_unique<handlers::SceneSubroutineHandler>(
                       gl::GL_FRAGMENT_SHADER, appImpl));
  uniformhdl = dynamic_cast<UniformHandler*>(gpi_->GetHandler(gl::GL_UNIFORM));
  outputhdl = dynamic_cast<ProgramOutputHandler*>(
      gpi_->GetHandler(gl::GL_PROGRAM_OUTPUT));
  ublockhdl = dynamic_cast<UniformBlockHandler*>(
      gpi_->GetHandler(gl::GL_UNIFORM_BLOCK));
  subroutinehdl = dynamic_cast<SubroutineUniformHandler*>(
      gpi_->GetHandler(gl::GL_FRAGMENT_SUBROUTINE_UNIFORM));
...
void IntrospectableFsq::Draw2D(bool* p_open) {
  ...
  gpi_->draw_gui(p_open,
                 {gl::GL_UNIFORM, gl::GL_PROGRAM_OUTPUT, gl::GL_UNIFORM_BLOCK,
...
void IntrospectableFsq::DrawToBackBuffer() { DrawFrame(*GetBackbuffer()); }
...
void IntrospectableFsq::DrawFrame(const viscom::FrameBuffer& fbo) {
  ...
  //            auto windowId =
  //            appImpl->GetApplication()->GetCurrentWindowIndex(); auto
  //            viewportPosition =
  //            appImpl->GetApplication()->GetViewportScreen(windowId).position_;
  //            auto viewportSize =
  //            appImpl->GetApplication()->GetViewportScreen(windowId).size_;
  //            auto viewportQuad =
  //            appImpl->GetApplication()->GetViewportQuadSize(windowId); auto
  //            vpScale =
  //            appImpl->GetApplication()->GetViewportScaling(windowId);

  // gl::glViewport(viewportPosition.x, viewportPosition.y, viewportSize.x,
  // viewportSize.y);
#endif
    gpi_->prepareDraw();
    gl::glUniform2ui(gpuProgram_->getUniformLocation("u_resolution"),
...
void IntrospectableFsq::miscinfo() {
  //        if
  //        (!ImGui::TreeNode(std::string("source##").append(fragmentShader).c_str()))
  //            return;
  //
  ...
  //            //ImGui::InputTextMultiline("##source", text.data(), shaderlen,
...
void IntrospectableFsq::init_callbacks() {
  ...
  outputhdl->post_init_fn =
      std::bind(&IntrospectableFsq::prog_out_hook, this, _1);
}

void IntrospectableFsq::init_hooks() {}

void IntrospectableFsq::prog_out_hook(
    std::vector<std::unique_ptr<named_resource>>& outputs) {
  if (outputs.size() != prev_backbuf_size) {
    auto value = viscom::FrameBufferTextureDescriptor(
        static_cast<GLenum>(gl::GLenum::GL_RGBA32F));
    // this assumes all outputs are vec4!
    backBuffers_ = appBase->CreateOffscreenBuffers(
        {{outputs.size(), value}, {/* no renderbuffers*/}});
  }

  auto& textureLocations =
      appBase->SelectOffscreenBuffer(backBuffers_)->GetTextures();
