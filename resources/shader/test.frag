...
vec2      iResolution = vec2(1920, 1080);           // viewport resolution (in pixels)
...
float fField(vec3 p)
{
   ...
   float sphere = length(p-vec3(1.+sin(iTime*.5)*.2,.8,1))-1.;
   float d;
   float r = 0.3;
   float n = 4.;
...
void main()
{
	float time = (1 + sin(2*iTime))/2;
	color = vec4(texCoord,fract(iTime),1);
