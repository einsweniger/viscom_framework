...
uniform vec3 bgcolor = vec3(0, 0.1, 0.1);
uniform vec3 color_a = vec3(1.0, 0.4, 0);
uniform vec3 color_b = vec3(0.3, 0, 0.7);
uniform vec3 outline = vec3(1, 0, 0.2);
...
float sample_dist_gaussian(vec2 uv) {
    ...
    for (int i=0; i<nstep; ++i) {
        for (int j=0; j<nstep; ++j) {
            ...
            float dist = texture(tex_text, uv-delta).w - TEX_BIAS;
            float wij = w[i]*w[j];
...
subroutine(SceneMap)
vec2 glyphmap(in vec3 pos) {

    int ca = int(global_t) % 26;
    int cb = (ca + 1) % 26;
    int cc = (ca + 2) % 26;
...
vec3 glyphray( in vec3 ro, in vec3 rd) {
...
vec3 shade( in vec3 ro, in vec3 rd ,
    ...
    if (tm.x >= 0.0) {
        ...
        float flip = mod(global_t, 2.0) < 1.0 ? -1.0 : 1.0;
        
        vec3 fg = mix(color_a, color_b, step(flip*d.x, flip*d.y));
                
        c = mix(fg, outline, smoothstep(0.003, 0.0, abs(d.x-d.y)-0.008));
...
float scribble(vec2 p, float k) {
    ...
    for (int i=-1; i<2; ++i) {
        for (int j=-1; j<2; ++j) {
            ...
            vec4 r = textureLod(tex_noise, (cij+0.5)/256., 0.);
            cij += 0.5*(r.xy - 0.5);
...
void glyphimage( out vec4 fragColor, in vec2 fragCoord ) {

    vec2 uv = (fragCoord.xy - .5*iResolution.xy) * scale / (iResolution.y);
    //uv = texCoord* 0.8 / (iResolution.y);

    global_t = 0.45*max(0.0, iTime-2.0);
    ...
    if (max(iMouse.x, iMouse.y) > x0) {        
        ...
        global_t = 6.5 * (ux + row) + 1e-5;

    }
    
    float u = fract(global_t);
