...
/*
What i've learnt:
length(max(abs(p)-b,0.0))  produces better edges than
vmax(abs(p) - b)

check if that is the general case and find out which is cheaper.
*/
...
//possibly identical, results look the same :O
...
float fBox(vec3 p, vec3 b) {
    vec3 d = abs(p) - b;
    return length(max(d,0.0)) + vmax(min(d, vec3(0)));
}  // Box: correct distance to corners


//identical for r=0, udRoundBox does produce better edges.
//radius is added to final dimensions, so the box has size b+vec3(r)
