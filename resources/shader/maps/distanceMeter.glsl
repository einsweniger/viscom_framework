...
vec3 distanceMeter(float distance_field, float rayLength, vec3 rayDir, float distance_to_cam) {
    float idealGridDistance = 20.0/rayLength*pow(abs(rayDir.y),0.8);
    float nearestBase = floor(log(idealGridDistance)/log(10.));
    float relativeDist = abs(distance_field/distance_to_cam);
    ...
    col = max(vec3(0.),col);
    if (sign(distance_field) < 0.) {
        col = col.grb*3.;
    }

    float l0 = (pow(0.5+0.5*cos(distance_field*PI*2.*smallerDistance),10.0));
    float l1 = (pow(0.5+0.5*cos(distance_field*PI*2.*largerDistance),10.0));
