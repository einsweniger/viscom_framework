// source: https://www.shadertoy.com/view/4scXzn
// author: Shane
/*
...
vec3 windingTunnel( vec3 p ) {
    ...
    return vec3(min(min(tube, walls), screen),objID*10,objID);
...
vec3 tunnel(in vec3 origin, in vec3 direction, out float distance) {
    vec3 lp = normalize(render_light_direction);
    vec3 rd = normalize(direction);

    // Initialize the scene color.
    vec3 col = vec3(0);
    vec3 distAndMat = raymarch(origin, direction);
    float t = distAndMat.x;
    ...
    if(t<trace_INF){
        ...
        //if(saveObjID<2.5) nor = texBump(tex_wood, pOffs*ts, nor, 0.002 + step(saveObjID, 1.5)*0.012);

        if(text_rotation > 12) {
            col = tex3D(tex_wood, pOffs*ts, nor); // Texture pixel at the scene postion.
        }
        ...
        if(saveObjID>1.5){
            ...
            rCol = tex3D(tex_wood, (pOffs + reflect(rd, nor))*ts, nor);
        ...
        if(saveObjID>2.5 && text_rotation > 8){
    ...
    col = mix(min(col, 1.), vec3(0), 1.-exp(-t*t/trace_INF/trace_INF*15.));//smoothstep(0., FAR-20., t)
