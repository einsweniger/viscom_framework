...
float map( in vec3 pos ) {
    ...
    q.x-=11.;
	d = min(pyramid(q,5.),d);
    d = min(-2.55+distance(pos.xyz,vec3(0.))/4.+pos.y-max(.1,texture(iChannel0,pos.xz/20.).x)*2.1,d);
...
vec3 render( in vec3 ro, in vec3 rd, in vec2 uv )
{
	...
	float latt1 = pow( length(lpos1-pos)*.1, 1.5+.2*abs(sin(iTime/2.)) );
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //Camera
	float radius = 50.;
	vec3 ro = orbit(PI/2.-.5,PI/2.+iTime,radius);
   // ro.z -= iTime*4.;
